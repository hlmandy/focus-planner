...
function toStudent(r: ThesisStudentRow): ThesisStudent {
  return {
    ...
    name: r.name,
    topic: r.topic,
    stage: r.stage as ThesisStudent['stage'],
...
export function thesisStudentRoutes(app: Hono, db: Database.Database) {
  ...
  app.get('/api/thesis-students/:id', c => {
    const row = db.prepare('SELECT * FROM thesis_students WHERE id = ?').get(c.req.param('id')) as
      | ThesisStudentRow
      | undefined
  ...
  app.post('/api/thesis-students', async c => {
    ...
    const err =
      requireFields(body, ['id', 'projectId', 'name']) || checkEnum(stage, THESIS_STAGES, 'stage')
    if (err) return c.json({ error: err }, 400)
    db.prepare(
      `INSERT INTO thesis_students (id, project_id, name, topic, stage, next_milestone, due_date, notes, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      body.id,
      body.projectId,
      body.name,
  ...
  app.patch('/api/thesis-students/:id', async c => {
    ...
    const row = db.prepare('SELECT * FROM thesis_students WHERE id = ?').get(id) as
      | ThesisStudentRow
      | undefined
    if (!row) return c.json({ error: 'Thesis student not found' }, 404)
    ...
    db.prepare(
      `UPDATE thesis_students SET name = ?, topic = ?, stage = ?, next_milestone = ?, due_date = ?, notes = ?, updated_at = ? WHERE id = ?`,
    ).run(
      next.name,
      next.topic ?? '',
      next.stage,
