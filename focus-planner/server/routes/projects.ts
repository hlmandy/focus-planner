...
export function projectRoutes(app: Hono, db: Database.Database) {
  ...
  app.post('/api/projects', async (c) => {
    ...
    db.prepare(`INSERT INTO projects (id, name, color, kind, status, goal, due_date) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(
      body.id, body.name, body.color, kind, status, body.goal ?? '', body.dueDate ?? ''
    )
  ...
  app.patch('/api/projects/:id', async (c) => {
    ...
    const err = checkEnum(next.kind, PROJECT_KINDS, 'kind')
      || checkEnum(next.status, PROJECT_STATUSES, 'status')
    if (err) return c.json({ error: err }, 400)

    db.prepare(`UPDATE projects SET name = ?, color = ?, kind = ?, status = ?, goal = ?, due_date = ? WHERE id = ?`).run(
      next.name, next.color, next.kind, next.status, next.goal ?? '', next.dueDate ?? '', id
    )
  ...
  app.post('/api/projects/:id/reassign-and-delete', async (c) => {
    ...
    if (body.targetProjectId === id) return c.json({ error: 'targetProjectId must differ from the project being deleted' }, 400)
    ...
    const tx = db.transaction(() => {
      db.prepare('UPDATE tasks SET project_id = ? WHERE project_id = ?').run(body.targetProjectId, id)
      db.prepare('UPDATE thesis_students SET project_id = ? WHERE project_id = ?').run(body.targetProjectId, id)
      db.prepare('UPDATE research_logs SET project_id = ? WHERE project_id = ?').run(body.targetProjectId, id)
      db.prepare('UPDATE pomodoro_sessions SET project_id = ? WHERE project_id = ?').run(body.targetProjectId, id)
