...
export function habitEntryRoutes(app: Hono, db: Database.Database) {
  ...
  app.get('/api/habit-entries/:id', c => {
    const row = db.prepare('SELECT * FROM habit_entries WHERE id = ?').get(c.req.param('id')) as
      | HabitEntryRow
      | undefined
  ...
  app.post('/api/habit-entries', async c => {
    ...
    ).run(body.id, body.habitId, body.date, jsonBool(body, 'done') ? 1 : 0)
  ...
  app.patch('/api/habit-entries/:id', async c => {
    ...
    const row = db.prepare('SELECT * FROM habit_entries WHERE id = ?').get(id) as
      | HabitEntryRow
      | undefined
    if (!row) return c.json({ error: 'Habit entry not found' }, 404)
    ...
    db.prepare('UPDATE habit_entries SET done = ? WHERE id = ?').run(next.done ? 1 : 0, id)
