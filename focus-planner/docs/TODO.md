...
- [ ] **研究日记支持编辑**：DiaryPage 只有 addResearchLog + deleteResearchLog，已有条目只读，无法修改内容
- [ ] **文献记录支持编辑**：LiteraturePage 同上
- [ ] **番茄钟记录支持修正**：PomodoroSession 创建后无法编辑（改项目/时长）或删除；前端无管理 UI，后端 DELETE 路由已存在但前端未接入
- [ ] **项目详情"查看更多"**：ProjectsPage 文献列表硬切 `.slice(0, 5)`、附件 `.slice(0, 10)`，无展开按钮

## P2：体验优化

- [ ] **文献记录结构化字段**：ResearchLogEntry 只有 `title/source/note/attachments`，缺少阅读状态、关键结论、下一步行动等科研常用字段
- [ ] **项目管理筛选器持久化**：type/status 筛选器是 `useState('all')`，刷新即丢失，应存 localStorage
...
- [ ] **已归档项目展示**：status='archived' 存在但侧栏和非项目视图无入口，归档后"消失"

## P3：搜索与导出

- [ ] **全局搜索**：跨项目/日记/文献/附件索引（后端 `search.ts` 已有基础 LIKE 查询，前端无搜索 UI）
- [ ] **搜索结果筛选**：按类型、项目、日期范围
- [ ] **附件索引页**：跨项目汇总所有已记录附件
- [ ] **单项目 Markdown 导出**：SummaryPage 目前只做单日总结，无全项目导出（任务树 + 日记 + 文献 + 附件）
