...
export function ProjectsPage() {
  ...
  const projectStats = useMemo(() => {
    ...
    return projects.items.map(project => {
      ...
      return {
        ...project,
        taskCount: projectTasks.length, doneCount, logCount: projectLogs.length,
        literatureCount, attachmentCount, studentCount: projectStudents.length,
  ...
  const openProjectOverview = () => { setProjectFilterId('all'); setProjectDetailId(null); setEditingProjectId(null) }
  ...
  const renderProjectTask = (task: Task, depth = 0): ReactNode => {
    const children = activeProjectTasks.filter(item => item.parentId === task.id).sort((a, b) => a.createdAt.localeCompare(b.createdAt))
    return (
  ...
  return (
    <div className="projects-page">
      ...
      <div className="project-composer">
        <input value={newProjectName} onChange={e => setNewProjectName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addProject()} placeholder="新课题 / 论文 / 指导事项 / 事务" />
      ...
      {projectDetailId === null ? (
        <div className="project-grid">
          {managedProjectStats.map(project => (
            <article key={project.id} className="project-card" onClick={() => { setProjectFilterId(project.id); setProjectDetailId(project.id) }}>
              ...
              <div className="project-card-stats">
                <span><strong>{project.taskCount}</strong>任务</span>
                <span><strong>{project.doneCount}</strong>完成</span>
                <span><strong>{durationText(project.weekFocusMinutes)}</strong>专注</span>
              </div>
              <div className="project-card-stats research-stats">
                {project.kind === 'student' ? (
                  <>
                    <span><strong>{project.studentCount}</strong>学生</span>
                    <span><strong>{project.taskCount}</strong>待办</span>
                    <span><strong>{durationText(project.weekFocusMinutes)}</strong>指导时间</span>
                  </>
                ) : (
                  <>
                    <span><strong>{project.logCount}</strong>日记</span>
                    <span><strong>{project.literatureCount}</strong>文献</span>
                    <span><strong>{project.attachmentCount}</strong>附件</span>
                  </>
                )}
              </div>
        ...
        <section className="project-detail">
          ...
          {editingProjectId === activeProjectStats.id ? (
            <section className="project-meta-editor">
              <label>名称<input value={activeProjectStats.name} onChange={e => updateProject(activeProjectStats.id, { name: e.target.value })} /></label>
              <label>类型<select value={activeProjectStats.kind} onChange={e => updateProject(activeProjectStats.id, { kind: e.target.value as ProjectKind })}>
                <option value="research">科研</option><option value="paper">论文</option><option value="student">指导</option><option value="admin">事务</option>
              </select></label>
              <label>状态<select value={activeProjectStats.status} onChange={e => updateProject(activeProjectStats.id, { status: e.target.value as ProjectStatus })}>
                <option value="active">进行中</option><option value="paused">暂停</option><option value="done">完成</option><option value="archived">归档</option>
              </select></label>
              <label>截止日期<input type="date" value={activeProjectStats.dueDate} onChange={e => updateProject(activeProjectStats.id, { dueDate: e.target.value })} /></label>
              <label className="project-goal-field">目标 / 说明<textarea value={activeProjectStats.goal} onChange={e => updateProject(activeProjectStats.id, { goal: e.target.value })} placeholder="这个项目的目标、范围、当前重点或注意事项" /></label>
            </section>
          ) : (
            <section className="project-meta-view">
              <span className={`kind-pill ${activeProjectStats.kind}`}>{projectKindLabels[activeProjectStats.kind]}</span>
              <span className={`status-pill ${activeProjectStats.status}`}>{projectStatusLabels[activeProjectStats.status]}</span>
          ...
          <div className="project-detail-grid">
            <section className={`project-panel template-panel ${activeProjectStats.kind}`}>
              ...
              {activeProjectStats.kind === 'research' && <div className="template-notes"><span>研究问题</span><span>文献基础</span><span>实验/分析</span><span>阶段结果</span></div>}
              {activeProjectStats.kind === 'paper' && <div className="template-notes"><span>论文结构</span><span>结果图表</span><span>写作推进</span><span>投稿准备</span></div>}
              {activeProjectStats.kind === 'student' && <div className="template-notes"><span>多学生进度</span><span>阶段节点</span><span>反馈重点</span><span>风险跟进</span></div>}
              {activeProjectStats.kind === 'admin' && <div className="template-notes"><span>待处理事务</span><span>会议沟通</span><span>材料提交</span><span>后续跟进</span></div>}
            </section>
            <section className="project-panel">
              <h3>子任务树</h3>
              ...
              <div className="project-task-list">
                {activeProjectTasks.length ? (
                  activeProjectTasks.filter(t => !t.parentId).sort((a, b) => a.createdAt.localeCompare(b.createdAt)).map(task => renderProjectTask(task))
            ...
            {activeProjectStats.kind === 'student' && (
              <section className="project-panel thesis-panel">
                ...
                <div className="thesis-composer">
                  <input value={newStudentName} onChange={e => setNewStudentName(e.target.value)} placeholder="学生姓名" />
                  <input value={newStudentTopic} onChange={e => setNewStudentTopic(e.target.value)} placeholder="论文题目 / 方向" />
                  <select value={newStudentStage} onChange={e => setNewStudentStage(e.target.value as ThesisStage)} aria-label="论文阶段">
                    ...
                    activeProjectStudents.map(student => (
                      <article key={student.id} className="thesis-student">
                        ...
                        <div className="thesis-row">
                          <input value={student.nextMilestone} onChange={e => updateThesisStudent(student.id, { nextMilestone: e.target.value })} placeholder="下个节点" />
                          <input type="date" value={student.dueDate} onChange={e => updateThesisStudent(student.id, { dueDate: e.target.value })} />
                        </div>
                        <textarea value={student.notes} onChange={e => updateThesisStudent(student.id, { notes: e.target.value })} placeholder="指导备注 / 风险 / 下次反馈重点" />
                        <div className="thesis-meta">{thesisStageLabels[student.stage]} · 截止 {student.dueDate || '未设置'}</div>
                      </article>
                    ))
                  ...
                  <div className="project-attachment-list">
                    {activeProjectAttachments.map(attachment => (
                      isWebLink(attachment.name) ? (
                        ...
                        <span key={attachment.id}><Paperclip size={13} /><span>{attachment.name}</span><em>{attachment.entryTitle}</em></span>
                      )
                    ))}
                  </div>
