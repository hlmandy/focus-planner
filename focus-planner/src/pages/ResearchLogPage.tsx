  ...
  return (
    <div className="research-log-page">
      <div className="log-toolbar">
        ...
        <label className="log-date-label">
          日期
        ...
        <select
          value={kindFilter}
          onChange={event => setKindFilter(event.target.value as ResearchLogKind | 'all')}
          aria-label="筛选类型"
        >
          {kindOptions.map(opt => (
      ...
      <div className="quick-capture">
        <div className="quick-capture-row">
          ...
          <select
            value={quickKind}
            onChange={event => setQuickKind(event.target.value as ResearchLogKind)}
            className="quick-capture-kind"
          >
            {kindOptions
          ...
          <button type="button" onClick={addQuickLog} className="btn btn-primary quick-capture-btn">
            <Plus size={17} />
          </button>
        </div>
      </div>
        ...
        {visibleLogs.length ? (
          visibleLogs.map(entry => {
            ...
            return (
              <article
                key={entry.id}
                className={`log-entry ${isExpanded ? 'expanded' : ''} ${isEditing ? 'editing' : ''}`}
              >
                {isEditing ? (
                  <>
                    ...
                    <label>
                      ...
                      <select
                        ...
                        onChange={e => setEditReadingStatus(e.target.value as ReadingStatus)}
                        aria-label="阅读状态"
                      >
                    ...
                    <textarea
                      className="log-edit-textarea compact"
                      value={editKeyFindings}
                      onChange={e => setEditKeyFindings(e.target.value)}
                      placeholder="核心发现 / 关键结论"
                    />
                    <input
                      className="log-edit-input"
                  ...
                  <>
                    <div className="log-entry-summary" onClick={() => toggleExpand(entry.id)}>
                      ...
                      <ChevronDown
                        size={15}
                        className={`log-expand-icon ${isExpanded ? 'open' : ''}`}
                      />
                      <div className="log-entry-actions" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          type="button"
                          onClick={() => deleteResearchLog(entry.id)}
                          aria-label="删除"
                    ...
                    {isExpanded && (
                      <div className="log-entry-detail">
                        ...
                        {entry.keyFindings && (
                          <p className="log-findings">
                            <strong>关键结论：</strong>
                            {entry.keyFindings}
                          </p>
                        )}
                        {entry.nextAction && (
                          <p className="log-next-action">
                            <strong>下一步：</strong>
                            {entry.nextAction}
                          </p>
                        )}
